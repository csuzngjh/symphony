defmodule SymphonyElixir.ControlPlane.PrFlowTest do
  ...
  test "commits, pushes, creates PR, and moves tracker state after agent completion" do
    ...
    command_runner = fn cmd, args, opts ->
      ...
      case {cmd, args} do
        ...
        {"git", ["add", "--", "lib/example.ex", "test/example_test.exs"]} ->
          {"", 0}

        {"git", ["commit", "-m", message]} ->
          assert message =~ "pri-170"
          {"[branch abc] commit", 0}
  ...
  test "does not stage the control-plane completion report" do
    parent = self()
    workspace_path = Path.join(System.tmp_dir!(), "symphony_pr_flow_completion_artifact_test")
    File.rm_rf!(workspace_path)
    File.mkdir_p!(workspace_path)
    ...
    command_runner = fn cmd, args, opts ->
      ...
      case {cmd, args} do
        {"git", ["status", "--porcelain"]} ->
          {" M lib/example.ex\n?? .symphony/agent-completion.json\n", 0}
